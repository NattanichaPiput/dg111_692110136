...
// รับคาตอบจากผู้เล่นแล้วเขียนลง *guess โดยตรง(ถ้าไม่อยู่ในช่วง 1-6 จะถามใหม่จนกว่าจะถูกต้อง)
...
void checkGuess(int dice, int guess, int *score)
{
    ...
    else if (abs(dice - guess) == 1) // abs ทำให้ผลต่างเป็นบวกเสมอ — ถ้า guess ใกล้เคียง +1 point
    {
...
int main()
{
    srand((unsigned)time(NULL)); // seed สำหรับ random number generator / time(NULL) ทำให้การันตีว่าแต่ละครั้งที่รันโปรแกรมจะได้ผลลัพธ์ที่ต่างกัน
